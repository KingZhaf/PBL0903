...
def cars_view(request):
    if request.method == 'GET':
        ...
        # Also mirror the entry into DB-B
        try:
            TBCarsWeb.objects.using('db_b').create(
                carname=fName,
                carbrand=fBrand,
                carmodel=fModel,
                carprice=fPrice,
            ...
            # best-effort mirror; ignore on failure but continue
            pass
...
def car_detail_view(request, car_id):
    ...
    if request.method == 'GET':
        ...
        # Best-effort: find a matching record in DB-B by old values and update it.
        try:
            match = TBCarsWeb.objects.using('db_b').filter(
        ...
        car.delete()
        # Best-effort: delete matching record(s) in DB-B
        try:
            TBCarsWeb.objects.using('db_b').filter(
                carname=old_values['carname'],
                carbrand=old_values['carbrand'],
                carmodel=old_values['carmodel'],
                carprice=old_values['carprice'],
